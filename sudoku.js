...
// 				// printSudoku(sudoku);
// 				//fill the next cell
// 				// var ret = backtrack(sudoku, list)//, row, col+1);
// 				list = backtrack(sudoku, list);
...
function createHTMLForInputs(){
  ...
  for(let k = 0; k < 9; k++){
    for(let l = 0; l < 9; l++){
      s += "<input type=\"text\" style=\"position:relative;left:" + left +"%;top:" + top + "%;width:25px; id=\"" + row + col + "\" maxlength=\"1\" size=\"1\"" + "/>";
      col++;
      left += 5.3;
    }
    row++;
